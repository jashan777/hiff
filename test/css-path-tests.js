...
describe("nodePath()", function () {
  ...
  it("should use nth-of-type() to disambiguate even if classes are present", function () {
    var html =
      '<a class="red">One.</a><a class="red">Two.</a><a class="red">Three.</a>';
    var $ = cheerio.load(html);
    var $secondA = $($("a")[1]);
  ...
  it("should return 'undefined' for nodes which don't have valid CSS paths", function () {
    ...
    var $ = cheerio.load(html);
    var contents = _.map($("div").contents(), function (node) {
      return $(node);
    });
  ...
  it("should produce paths that can be fed back to $()", function (done) {
    fs.read(path.join(__dirname, "fixtures/path-roundtrip-test.html"))
      .then(function (html) {
        var $ = cheerio.load(html);
        ...
        $("*").each(function () {
          ...
          if ($.html($node) != $.html($(path))) {
            throw new AssertionError(
              "Path '" + path + "' is incorrect for node:\n" + $.html($node)
            );
          }
        });
      })
