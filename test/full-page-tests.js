...
describe("Hiff", function () {
  it("should deal with completely replacing content of a deeply nested tag", function () {
    var d = compareFixtures(
      "replaced-content-before.html",
      "replaced-content-after.html"
    );
    ...
    _.each(d.changes, function (c) {
      ...
      assert.equal(
        c.after.parentPath,
        "html > body > div#page-wrapper > section#content"
      );
    });
  });
  ...
  it("should deal with content with special comparison requirements", function () {
    var d = compareFixtures(
      "custom-logic-before.html",
      "custom-logic-after.html",
      {
    ...
    var articleChanges = _.filter(d.changes, function (c) {
      return (
        (c.before.$node && c.before.$node.is("article")) ||
        (c.after.$node && c.after.$node.is("article"))
      );
    });
    assert.lengthOf(articleChanges, 4);
    assert(
      _.every(articleChanges, { type: "changed" }),
      "There were additions/removals for <article> tags."
    );
  });

  it("paragraph soups with no classes or IDs", function () {
    var d = compareFixtures(
      "paragraph-soup-before.html",
      "paragraph-soup-after.html"
    );
    assert.ok(d.different);
    assert(
      _.every(d.changes, { type: "added" }),
      "There were non-addition changes."
    );
    assert(
      _.every(d.changes, function (c) {
