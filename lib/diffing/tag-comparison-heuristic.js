...
var canonicalizeAttribute =
  require("../util/cheerio-utils").canonicalizeAttribute;
...
var DEFAULT_OPTIONS = {
  ...
  id: 1.0,
  attributes: 1.0,
  contents: 1.0, // weight multipliers
};
...
function createHeuristic(weights) {
  ...
  function heuristic($n1, $n2, childChanges) {
    ...
    if (attributes.indexOf("id") >= 0) {
      ...
      var idsDiffer = attributeValuesDiffer(
        $n1[0].attribs["id"],
        $n2[0].attribs["id"]
      );
    ...
    if (attributes.length) {
      ...
      different =
        different || (differentAttribs.length > 0 && weights.attributes > 0);
      similarity += componentResult(
        "attributes",
        attributesDifferSignificantly
      );
    }

    // finally, if the contents are at least 50% different
    // we treat this as a significant difference
    var possibleChildChanges = _.max([
      $n1.contents().length,
      $n2.contents().length,
    ]);
    if (possibleChildChanges > 0) {
      ...
      var contentsDifferSignificantly =
        totalChildChanges / possibleChildChanges > 0.99;
    ...
    if (!different) {
      ...
      return similarity >= 0
        ? DiffLevel.SAME_BUT_DIFFERENT
        : DiffLevel.NOT_THE_SAME_NODE;
    }
  }
  ...
  function componentResult(component, isDifferent) {
    return (
      HEURISTIC_WEIGHTS[component][isDifferent ? "differs" : "same"] *
      weights[component]
    );
  }
}
