...
function compareNodes($n1, $n2, options) {
  ...
  if (options.ignore) {
    if (isIgnored($n1) && isIgnored($n2)) return false;
  }
  ...
  function findDifferences($n1, $n2, options) {
    ...
    switch (type1) {
      case "text":
        return compareTextNodes($n1, $n2, options.ignoreText);
      case "element":
        return compareTags($n1, $n2, options.tagComparison);
      case "directive":
      case "comment":
  ...
  function compareNodeLists($parent1, $parent2, list1, list2, options) {
    ...
    _.each(parts, function (part) {
      ...
      if (!part.added && !part.removed) {
        var nodesToCheck = _.zip(
          list1.slice(index1, index1 + part.count),
          list2.slice(index2, index2 + part.count)
        );
        index1 += part.count;
        index2 += part.count;
        ...
        changes = changes.concat(
          addedNodes.map(function ($node, offset) {
        ...
        changes = changes.concat(
          removedNodes.map(function ($node, offset) {
  ...
  function compareTextNodes($n1, $n2, ignoredParents) {
    ...
    if (textNodesShouldBeIgnored($n1, $n2, ignoredParents)) return false;
